...
public class LoginController extends Server {
    ...
    public Message login(String username, String password, Client client) throws Exception {
        ...
        client.writeMessage(new Message("login"));
        client.readMessage();
        String path = "src/main/resources/Images/" + account.getImagePath();
        client.writeMessage(new Message(path));
        Message toClient = client.readMessage();
        if (toClient.getText().equals("has file"))
            client.writeMessage(new Message("okay"));
        else
